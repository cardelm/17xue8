...
if(!submitcheck('submit')) {
	...
	$shopsorts = C::t(GM('server_goodssort'))->range();
	foreach($shopsorts as $k=>$v ){
		if($v['sortupid']==0){
			$sorts[$v['goodssortid']] = $v;
			foreach($shopsorts as $k1=>$v1 ){
				if($v1['sortupid']==$v['goodssortid']){
					$sorts[$v['goodssortid']]['subsort'][] = $v1;
				}
			}
			$sorts[$v['goodssortid']]['subsort'] = array_sort($sorts[$v['goodssortid']]['subsort'],'displayorder','asc');
		}
	}

	foreach($sorts as $k=>$row ){
		showtablerow('', array('class="td25"','style="width:120px"','class="td23"', '', ''), array(
			(count($row['subsort']) ? '<a href="javascript:;" class="right" onclick="toggle_group(\'subnav_'.$k.'\', this)">[+]</a>' : '').(count($row['subsort']) ? '<input type="checkbox" class="checkbox" value="" disabled="disabled" />' : "<input class=\"checkbox\" type=\"checkbox\" name=\"delete[]\" value=\"$row[goodssortid]\" />"),
			'<input type="text" name="displayordernew['.$row['goodssortid'].']" value="'.$row['displayorder'].'" size="2" />&nbsp;&nbsp;('.count($row['subsort']).')',
			"<input type=\"text\" class=\"txt\" size=\"15\" name=\"namenew[$row[goodssortid]]\" value=\"".$row['sortname']."\">",
			"<div><input type=\"text\" class=\"txt\" size=\"15\" name=\"titlenew[$row[goodssortid]]\" value=\"".$row['sorttitle']."\"><input type=\"hidden\" name=\"upsortid[$row[goodssortid]]\" value=\"0\"><a href=\"###\" onclick=\"addrowdirect=1;addrow(this, 1, '$row[goodssortid]')\" class=\"addchildboard\">".lang('plugin/yiqixueba','add_subsort')."</a></div>",

		));
			showtagheader('tbody', 'subnav_'.$k,false);
			foreach ($row['subsort']  as $kk => $subrow ){
				showtablerow('', array('class="td25"','style="width:120px"','class="td23"', '', ''), array(
					"<input class=\"checkbox\" type=\"checkbox\" name=\"delete[]\" value=\"$subrow[goodssortid]\">",
					"<div class=\"board\"><input type=\"text\" name=\"displayordernew[$subrow[goodssortid]]\" value=\"$subrow[displayorder]\" size=\"2\" /></div>",
					"<input type=\"text\" class=\"txt\" size=\"15\" name=\"namenew[$subrow[goodssortid]]\" value=\"".$subrow['sortname']."\">",
					"<input type=\"text\" class=\"txt\" size=\"15\" name=\"titlenew[$subrow[goodssortid]]\" value=\"".$subrow['sorttitle']."\"><input type=\"hidden\" name=\"upsortid[$subrow[goodssortid]]\" value=\"".$subrow['sortupid']."\">",
				));
			}
			showtagfooter('tbody');
	}
...
EOT;
		...
		foreach( getgpc('delete') as $k=>$v ){
			if($v){
				C::t(GM('server_goodssort'))->delete($v);
			}
		}
		//原数据提交更新仅一级分类数据
		if(is_array($_GET['namenew'])) {
			foreach($_GET['namenew'] as $k => $v) {
				...
				if($v && $titlenew){
					...
					C::t(GM('server_goodssort'))->update($k,$data);
				}
			}
		}
		//新建一级分类
		if(is_array($_GET['newname'])) {
			foreach($_GET['newname'] as $k => $v) {
				...
				if($v && $newtitle){
					...
					$data['upids'] = '';
					C::t(GM('server_goodssort'))->insert($data);
				}
			}
		}
		//新建二级分类
